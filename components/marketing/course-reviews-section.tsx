 ...
 *
 * Shows approved student reviews (from `get_course_public_reviews`) as a
 * centered, editorial-style testimonial column under a compact rating summary
 * bar. The centered layout reads as deliberate whether there's one review or
 * many — no sparse half-empty grid. Renders nothing when there are no reviews.
 *
  ...
  return (
    ...
    >
      ...
      <div className="container-narrow relative">
        ...
        {/* Rating summary bar */}
        <div className="mx-auto mb-10 flex max-w-2xl flex-col gap-6 rounded-3xl bg-cream-50 p-6 shadow-soft ring-1 ring-charcoal-100/40 sm:flex-row sm:items-center sm:gap-10 sm:p-8 md:mb-12">
          <div className="shrink-0 text-center sm:text-left">
            <div className="font-serif text-5xl leading-none text-charcoal-600">
              {avg.toFixed(1)}
            </div>
          ...
          <div className="hidden h-16 w-px shrink-0 bg-charcoal-100/60 sm:block" />

          <div className="min-w-0 flex-1 space-y-1.5">
            {distribution.map(({ star, n, pct }) => (
              <div key={star} className="flex items-center gap-3 text-xs">
                <span className="flex w-7 shrink-0 items-center gap-0.5 text-charcoal-400">
                  {star}
                ...
                <span className="w-4 shrink-0 text-right tabular-nums text-charcoal-400">
                  {n}
                </span>
              </div>
            ))}
...
function ReviewCard({
  ...
  return (
    <motion.figure
      ...
      transition={{ duration: 0.6, delay: Math.min(index, 4) * 0.08, ease: EASE }}
      className={`group relative overflow-hidden rounded-2xl bg-cream-50 p-6 shadow-soft ring-1 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-soft-lg sm:p-8 ${
        review.is_pinned
          ? "ring-mustard/40"
    ...
    >
      <Quote
        aria-hidden="true"
        className="absolute -right-2 -top-2 h-20 w-20 text-mustard/[0.07]"
      />

      {review.is_pinned && (
        <span className="absolute right-5 top-5 inline-flex items-center gap-1 rounded-full bg-mustard px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-white shadow-soft">
      ...
      {review.title && (
        <h3 className="relative mt-4 font-serif text-xl leading-snug text-charcoal-600">
          {review.title}
        </h3>
      )}

      {review.body && (
        <blockquote className="relative mt-3 text-base leading-relaxed text-charcoal-600/90 sm:text-lg">
          {review.body}
        </blockquote>
      )}

      <figcaption className="relative mt-6 flex items-center gap-3 border-t border-charcoal-100/50 pt-5">
        {review.reviewer_avatar ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={review.reviewer_avatar}
            alt=""
            className="h-10 w-10 rounded-full object-cover ring-1 ring-charcoal-100/50"
          />
        ) : (
          <span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-mustard/15 text-sm font-semibold text-mustard-600">
            {initials}
          </span>
        )}
        <div className="min-w-0">
          ...
          <p className="text-xs text-charcoal-400">Verified student · {date}</p>
        </div>
      </figcaption>
    </motion.figure>
  );
